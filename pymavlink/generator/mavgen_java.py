...
def generate_tests(basename, xml):
    '''generate per-message unit tests'''
    directory = os.path.join(basename, '''test''')
    mavparse.mkdir_p(directory)
    for m in xml.message:
        f = open(os.path.join(directory, 'test_msg_%s.java' % m.name_lower), mode='w')
        path=directory.split('/')
        t.write(f, '''
/* AUTO-GENERATED FILE.  DO NOT MODIFY.
 *
...
public class test_msg_${name_lower} extends MAVLinkMessage{

    long time_usec = 123456789;//generate random numbers here
    float controls[] = new float[]{1,2,3,4,5,6,7,8};
    public short group_mlx = 25000;

    @Test
    public void test(){
	//Use the generated class to generate the packet
	...
	byte[] encodedPacket = mAVLinkPacket.encode();

	//generate the packet manually
	byte[] manuallyEncodedPacket = new byte[]{0,0,0,0,0};

	assertEquals("msg_${name_lower} was not generated correctly", 1,1);
    }
}
    ''', m)
    f.close()
